...
이 문서는 tossinvest-sdk SDK의 **로컬 CPU 벤치마크**, **테스트 커버리지**, **정적 품질 메트릭**을 기록합니다. HTTP API 클라이언트이므로 실제 엔드투엔드 latency는 네트워크/토스증권 서버에 의존하지만, SDK 자체가 요청·응답마다 수행하는 직렬화·역직렬화 오버헤드는 로컬로 측정 가능하고 회귀 추적 가치가 있습니다.
