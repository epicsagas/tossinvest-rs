...
> 🧪 **플라잇 테스트 미검증 (Pre-production)**
> 본 SDK는 목업(wiremock) 기반 단위·E2E 테스트로 검증되었으나, **실제 토스증권 API 연동(flight test)은 아직 수행되지 않았습니다.** 프로덕션 환경에 도입하기 전에 반드시 실제 자격증명으로 충분한 통합 테스트를 거쳐야 하며, 스펙과 응답 형식 차이로 인한 예상치 못한 동작이 있을 수 있으니 감안해서 사용해 주세요.
