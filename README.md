...
> ⚠️ **중요 공지 (Disclaimer)**
> 본 라이브러리는 개인이 개발한 **비공식(Unofficial)** Rust SDK이며, [토스증권](https://tossinvest.com)(Toss Invest)의 공식 지원이나 관리를 받지 않는 독립적인 오픈소스 프로젝트입니다.
> '토스증권' 및 'Toss' 관련 상표명에 대한 권리는 비바리퍼블리카 / 토스증권에 있습니다.
> 공식 정보 및 문서는 [토스증권 Open API 문서](https://developers.tossinvest.com/)를 참고하시기 바랍니다.

> 🧪 **프로덕션 사용 주의**
> 이 SDK는 실제 토스증권 운영 환경(라이브 API) 통합 테스트를 거치지 않았습니다. 모든 테스트는 wiremock 기반 모킹으로 수행되었으므로, 실제 자금이 오가는 프로덕션 도입 전 반드시 라이브 환경에서 사전 검증을 거치시기 바랍니다. **본 라이브러리 사용으로 인해 발생하는 손실에 대해 개발자는 책임을 지지 않습니다.**

> 🚫 **면책 조항 (Liability)**
> 본 라이브러리를 사용하다가 발생하는 금융적·물질적 손실에 대해 원작자 및 해당 기업(토스증권)은 일절 책임지지 않습니다.
